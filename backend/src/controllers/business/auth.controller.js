    ...
    try {
        ...
        const bizResult = await client.query(
            ...
            [owner_id, business_type, business_name, description || null, gst_number || null,
             year_established || null, logoUrl, coverUrl,
             latitude || null, longitude || null, full_address || null, city || null, state || null,
             pincode || null, nearest_station_id || null, opening_time || null, closing_time || null,
             days_open ? JSON.parse(days_open) : null, closed_on_holidays === 'true',
             payment_modes ? JSON.parse(payment_modes) : null]
        );
        ...
        if (business_type === 'restaurant') {
            ...
            await client.query(
                ...
                [business_id, total_rooms || null, star_rating || null,
                 check_in_time || null, check_out_time || null,
                 extra_bed_available === 'true', extra_bed_charge || null,
                 amenities ? JSON.parse(amenities) : null]
            );

            if (room_types) {
                const parsedRooms = JSON.parse(room_types);
                for (const room of parsedRooms) {
                    await client.query(
                        `INSERT INTO room_types (business_id, room_type, price_per_night, total_rooms_of_type, extra_bed_available, extra_bed_charge)
                         VALUES ($1,$2,$3,$4,$5,$6)`,
                        [business_id, room.room_type, room.price_per_night, room.total_rooms_of_type || null, room.extra_bed_available || false, room.extra_bed_charge || null]
                    );
                }
            }
        }
...
const login = async (req, res) => {
    try {
        ...
        const result = await db.query(
            `SELECT bo.*, b.id as business_id, b.business_type, b.status, b.rejection_reason, b.business_name
        ...
        if (owner.status === 'pending') {
            return res.status(403).json({ success: false, error: { message: 'Your account is pending admin approval.' } });
        }
        if (owner.status === 'rejected') {
            return res.status(403).json({ success: false, error: { message: 'Your account was rejected.', reason: owner.rejection_reason } });
        }
        ...
        res.json({
            ...
            business: { id: owner.business_id, name: owner.business_name, type: owner.business_type, status: owner.status }
        });
    } catch (err) {
